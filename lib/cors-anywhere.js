...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  if (!requestState.redirectCount_) {
    res.setHeader('x-request-url', requestState.location.href.split('&api_key')[0]);
  }
  // Handle redirects
  ...
  proxyRes.headers['x-final-url'] = requestState.location.href.split('&api_key')[0];
...
function parseURL(req_url) {
  ...
  var parsed = url.parse(req_url + '&api_key=VcW9MiiefuESE91ta47k');
  if (!parsed.hostname) {
